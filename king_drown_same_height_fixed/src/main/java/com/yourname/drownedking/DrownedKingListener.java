...
public class DrownedKingListener implements Listener {
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        if (lastDamageCause instanceof EntityDamageByEntityEvent) {
            ...
            // 检查是否是溺尸王造成的死亡
            if (isDrownedKing(damager)) {
                Drowned boss = (Drowned) damager;
                manager.handlePlayerDeath(player, boss, true);
            }
            
            // 检查是否是溺尸守卫造成的死亡
            else if (damager instanceof Drowned && isMinion((Drowned) damager)) {
                Drowned minion = (Drowned) damager;
                manager.handleMinionKillPlayer(player, minion);
            }
            
            // 检查是否是Boss的三叉戟造成的死亡
            else if (damager instanceof Trident) {
                Trident trident = (Trident) damager;
                if (isBossTrident(trident)) {
                    manager.handleTridentKillPlayer(player, trident);
                }
            }
        }
    }
    ...
    private boolean isBossTrident(Trident trident) {
        return trident.getPersistentDataContainer().has(
