...
public class DrownedKingManager {
    ...
    private void summonMinions(Drowned boss, UUID bossId) {
        ...
        for (int i = 0; i < minionCount; i++) {
            Location spawnLoc = findSpawnLocation(bossLoc, 3, 8);
            if (spawnLoc != null) {
    ...
    private void teleportToRandomPlayer(Drowned boss) {
        ...
        Location newLocation = findSpawnLocation(target.getLocation(), 10, 20);
    ...
    public void handlePlayerDeath(Player player, Drowned boss) {
        String bossIdStr = boss.getPersistentDataContainer().get(
            new NamespacedKey(plugin, "drowned_king_boss"),
            PersistentDataType.STRING
        );
    ...
    private void handleBossRetreatAfterKill(Drowned boss, UUID bossId, String playerName) {
        ...
        Bukkit.getScheduler().runTaskLater(plugin, () -> {
            ...
            // 移除Boss
            forceRemoveBoss(boss, bossId, "§4溺尸王 §c在嘲笑声中退回了深渊...");
        }, 60L); // 3秒后退场
    }
    ...
    private void giveRewards(Drowned boss) {
        ...
        deathLocation.getWorld().dropItemNaturally(deathLocation, new ItemStack(Material.NETHERITE_INGOT, 20));
        deathLocation.getWorld().dropItemNaturally(deathLocation, new ItemStack(Material.DIAMOND, 50));
    }
