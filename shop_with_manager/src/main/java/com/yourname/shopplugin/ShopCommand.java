...
public class ShopCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleAdd(CommandSender sender, String[] args) {
        ...
        try {
            ...
            if (!isValidCategory(category)) {
                player.sendMessage(ChatColor.RED + "❌ 无效的分类！可用分类: " + String.join(", ", getCategoryKeys()));
                return;
            }
    ...
    private boolean isValidCategory(String category) {
        return shopManager.getCategories().containsKey(category);
    }
    
    // 修复：获取所有分类键
    private List<String> getCategoryKeys() {
        return new ArrayList<>(shopManager.getCategories().keySet());
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            ...
            // 修复：使用分类键而不是显示名称
            completions.addAll(getCategoryKeys());
        }
