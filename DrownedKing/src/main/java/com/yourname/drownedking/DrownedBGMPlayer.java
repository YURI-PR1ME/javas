...
public class DrownedBGMPlayer {
    ...
    public enum BGMTrack {
        DROWNED_PHASE_1("yourplugin:drowned_phase1", 160, SoundCategory.MUSIC, 1000.0f),  // 正常阶段
        DROWNED_FRENZY_PHASE("yourplugin:drowned_frenzy", 140, SoundCategory.MUSIC, 1000.0f); // 狂欢节阶段
    ...
    public enum BossPhase {
        DROWNED_NORMAL(BGMTrack.DROWNED_PHASE_1),
        DROWNED_FRENZY(BGMTrack.DROWNED_FRENZY_PHASE);
    ...
    private BossPhase currentPhase = BossPhase.DROWNED_NORMAL;
    private boolean isPlaying = false;
    private String currentSoundName = "";
    
    public DrownedBGMPlayer(DrownedKingPlugin plugin) {
        this.plugin = plugin;
    }
            ...
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                synchronized (lock) {
                    ...
                    if (bossWorldPlayers.isEmpty()) {
                        plugin.getLogger().info("没有玩家在溺尸王世界，跳过BGM播放");
                    ...
                    plugin.getLogger().info("开始播放溺尸王BGM: " + track.getSoundName() + 
    ...
    private void stopAllBGMInternal() {
        ...
        plugin.getLogger().info("已使用stopSound方法停止所有玩家的溺尸王BGM");
    }
    ...
    public void stopAllBGM() {
        synchronized (lock) {
            ...
            // 广播停止
            Bukkit.broadcastMessage("§7[音乐] 溺尸王战斗BGM已停止");
            plugin.getLogger().info("溺尸王BGM已停止");
        }
    }
    ...
    private List<Player> getPlayersInBossWorld() {
        ...
        // 查找溺尸王所在的世界
        for (DrownedKingBoss boss : plugin.getDrownedKingManager().getActiveBosses().values()) {
            org.bukkit.entity.Entity entity = Bukkit.getEntity(boss.getEntityId());
            if (entity != null && !entity.isDead()) {
                World bossWorld = entity.getWorld();
                for (Player player : Bukkit.getOnlinePlayers()) {
                    ...
                    Bukkit.getScheduler().runTaskLater(plugin, () -> {
                        if (player.isOnline()) {
                            ...
                            plugin.getLogger().info("为玩家 " + player.getName() + 
                                " 播放溺尸王跟随式BGM: " + track.getSoundName() + 
                                " (使用实体绑定模式)");
                        }
                    }, 1L);
    ...
    private void startGlobalBGMLoop(BGMTrack track) {
        ...
        globalBgmTask = new BukkitRunnable() {
            ...
            public void run() {
                synchronized (lock) {
                    ...
                    List<Player> bossWorldPlayers = getPlayersInBossWorld();
                    if (bossWorldPlayers.isEmpty()) {
                        plugin.getLogger().info("没有玩家在溺尸王世界，暂停BGM循环");
                        return;
                    }
                    ...
                    plugin.getLogger().fine("溺尸王BGM循环播放中... 当前阶段: " + currentPhase + 
                        ", 玩家数量: " + bossWorldPlayers.size());
                }
            }
        };
        ...
        plugin.getLogger().info("溺尸王BGM循环任务已启动，间隔: " + delayTicks + " ticks");
    }
    ...
    public void stopBGMForPlayer(Player player) {
        synchronized (lock) {
            stopSoundCompletely(player);
            plugin.getLogger().info("停止玩家 " + player.getName() + " 的溺尸王BGM");
            player.sendMessage("§7[音乐] 溺尸王战斗BGM已停止");
        }
    }
    ...
    public void updateBossPhase(BossPhase newPhase) {
        synchronized (lock) {
            ...
            plugin.getLogger().info("溺尸王BGM阶段切换: " + currentPhase + " -> " + newPhase);
    ...
    private void broadcastBGMStart(BossPhase phase) {
        ...
        switch (phase) {
            case DROWNED_NORMAL:
                message = "§7[音乐] §3ABYSS CALLING";
                break;
            case DROWNED_FRENZY:
                message = "§7[音乐] §cTIDAL FRENZY";
                break;
            default:
                message = "§7[音乐] §bDROWNED KING";
        }
        ...
        plugin.getLogger().info("溺尸王BGM已播放: " + message);
    }
    ...
    public void onPlayerLeaveBossWorld(Player player, World leftWorld) {
        synchronized (lock) {
            ...
            if (bossWorld != null && leftWorld.equals(bossWorld)) {
                ...
                plugin.getLogger().info("玩家 " + player.getName() + " 离开溺尸王世界，停止BGM");
            }
        }
    }
    
    /**
     * 当玩家进入溺尸王世界时，如果BGM正在播放，为其单独播放
     */
        ...
        synchronized (lock) {
            if (isPlaying && !isTransitioning) {
                ...
                if (bossWorld != null && enteredWorld.equals(bossWorld)) {
                    ...
                    player.sendMessage("§7[音乐] 溺尸王战斗BGM正在播放中...");
                }
            }
        }
    }
    ...
    public void testBGM(Player player) {
        if (isPlaying && !isTransitioning) {
            ...
            player.sendMessage("§e[测试] 播放溺尸王BGM测试...");
        ...
        if (isPlaying && !isTransitioning) {
            ...
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                ...
                if (!players.isEmpty()) {
                    ...
                    player.sendMessage("§a[音乐] 溺尸王BGM音量已调整为: " + newVolume);
                }
            }, 10L);
        }
    }
    ...
    public void cleanup() {
        synchronized (lock) {
            stopAllBGM();
            plugin.getLogger().info("溺尸王BGM播放器已清理");
        }
    }
}
