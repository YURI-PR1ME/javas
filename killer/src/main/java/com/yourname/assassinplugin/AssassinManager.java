...
public class AssassinManager {
    ...
    private LivingEntity spawnTier3Assassin(Location location, Player target) {
        ...
        assassin.getAttribute(Attribute.ATTACK_DAMAGE).setBaseValue(8.0); // 降低近战伤害，主要使用电涌攻击
    ...
    private void setDrownedTarget(LivingEntity assassin, Player target) {
        ...
        // 创建电涌攻击监控任务 - 移除时间限制，基于目标状态判断
        BukkitRunnable aiTask = new BukkitRunnable() {
            private long lastThrowTime = 0;
            private long lastShockTime = 0;
            private long spectatorTime = 0; // 记录目标处于旁观者模式的时间
            private boolean isTargetSpectator = false;
            private final long THROW_COOLDOWN = 40; // 2秒投掷冷却（40 ticks）
            private final long SHOCK_COOLDOWN = 7; // 约1秒3次电涌攻击（7 ticks）
            private final long SPECTATOR_TIMEOUT = 40; // 2秒旁观者超时（40 ticks）
            
            @Override
            public void run() {
                // 检查溺尸王是否有效
                if (!drowned.isValid() || drowned.isDead()) {
                    cleanupDrownedAI(drowned.getUniqueId());
                    this.cancel();
                    return;
                }
                ...
                boolean currentlySpectator = (targetGameMode == GameMode.SPECTATOR);
                
                if (currentlySpectator) {
                    // 目标处于旁观者模式
                    if (!isTargetSpectator) {
                        // 刚开始旁观，记录时间
                        isTargetSpectator = true;
                        spectatorTime = 0;
                    } else {
                        // 持续旁观，增加时间
                        spectatorTime++;
                        ...
                        // 如果旁观超过2秒，退场
                        if (spectatorTime >= SPECTATOR_TIMEOUT) {
                            cleanupDrownedAI(drowned.getUniqueId());
                            drowned.remove();
                            
                            // 通知雇主
                            Player employer = getContractEmployer(drowned);
                            if (employer != null) {
                                employer.sendMessage("§8[暗网] §7目标已长时间处于旁观模式，溺尸王退场");
                            }
                    ...
                    // 在旁观期间，不进行攻击，但保持追踪
                    drowned.setTarget(null); // 清除目标避免攻击
                    return;
                } else {
                    // 目标不是旁观者模式
                    if (isTargetSpectator) {
                ...
                if (distance <= 5.0) {
                    ...
                    if (currentTime - lastShockTime > SHOCK_COOLDOWN * 50) { // 转换为毫秒
                        performShockAttack(drowned, target);
                        lastShockTime = currentTime;
                    }
                } else {
                    // 5格外使用投掷攻击
                    long currentTime = System.currentTimeMillis();
                    if (currentTime - lastThrowTime > THROW_COOLDOWN * 50) { // 转换为毫秒
                        // 检查视线是否畅通
                        if (drowned.hasLineOfSight(target)) {
                            throwTrident(drowned, target);
                ...
                // 检查三叉戟数量限制
                checkTridentLimit(drowned.getUniqueId());
            }
        };
    ...
    // 检查并限制三叉戟数量
    private void checkTridentLimit(UUID drownedId) {
        List<UUID> tridents = drownedTridents.get(drownedId);
        if (tridents == null) return;
        
        // 限制最大10把三叉戟
        int maxTridents = 10;
        if (tridents.size() > maxTridents) {
            // 删除最早的三叉戟
            UUID oldestTridentId = tridents.remove(0);
            Entity oldestTrident = Bukkit.getEntity(oldestTridentId);
            if (oldestTrident != null && oldestTrident.isValid()) {
                oldestTrident.remove();
            }
        }
    }
    ...
    // 获取溺尸王对应的合约雇主
    private Player getContractEmployer(Drowned drowned) {
        String contractIdStr = drowned.getPersistentDataContainer().get(
            new NamespacedKey(AssassinPlugin.getInstance(), "assassin_contract"),
            PersistentDataType.STRING
        );
        ...
        return Bukkit.getPlayer(contract.getEmployerId());
    }
    
    // 执行电涌攻击
    private void performShockAttack(Drowned drowned, Player target) {
        if (drowned == null || target == null || drowned.isDead() || target.isDead()) return;
        
        try {
            // 造成2点伤害
            target.damage(2.0, drowned);
            
            // 播放电击效果
            target.getWorld().playSound(target.getLocation(), Sound.ENTITY_LIGHTNING_BOLT_THUNDER, 0.5f, 1.5f);
            
            // 增加电击计数
            int shockCount = drownedShockCounters.getOrDefault(drowned.getUniqueId(), 0) + 1;
            drownedShockCounters.put(drowned.getUniqueId(), shockCount);
            
            // 每6次电击触发一次雷击（恢复原功能）
            if (shockCount % 6 == 0) {
                triggerLightningStrike(drowned, target);
            }
            
            // 每60次电击触发三叉戟节日 - 修复：每次达到60的倍数都触发
            if (shockCount % 60 == 0) {
                triggerTridentFestival(drowned, target);
                // 注意：这里不重置计数，让计数继续增长，这样每60次都会触发
            }
            
            // 发送电击消息给目标
            if (shockCount % 10 == 0) { // 每10次电击发送一次消息，避免消息过多
                target.sendMessage("§b⚡ 被溺尸王电涌击中！ (" + shockCount + "/" + (shockCount / 60 * 60 + 60) + ")");
            }
            
        } catch (Exception e) {
            AssassinPlugin.getInstance().getLogger().warning("执行电涌攻击时出错: " + e.getMessage());
        }
    }
    
    // 触发雷击（恢复原功能）
    private void triggerLightningStrike(Drowned drowned, Player target) {
        if (drowned == null || target == null || drowned.isDead() || target.isDead()) return;
        
        try {
            // 生成闪电（但不造成伤害，因为我们自己控制伤害）
            LightningStrike lightning = target.getWorld().strikeLightningEffect(target.getLocation());
            
            // 造成额外伤害（7点）
            target.damage(7.0, drowned);
            
            // 播放更强的音效
            target.getWorld().playSound(target.getLocation(), Sound.ENTITY_LIGHTNING_BOLT_IMPACT, 1.0f, 1.0f);
            
            // 发送雷击消息
            //target.sendMessage("§c☇ 溺尸王召唤了雷电！");
            
        } catch (Exception e) {
            AssassinPlugin.getInstance().getLogger().warning("触发雷击时出错: " + e.getMessage());
        }
    }
    
    // 触发三叉戟节日 - 引爆所有三叉戟
    private void triggerTridentFestival(Drowned drowned, Player target) {
        if (drowned == null || target == null || drowned.isDead()) return;
        
        try {
            List<UUID> tridents = drownedTridents.get(drowned.getUniqueId());
            if (tridents == null) tridents = new ArrayList<>();
            
            // 检查三叉戟数量，如果不足5根，立即发射足够的三叉戟
            int currentTridents = tridents.size();
            int minTridents = 5;
            
            // 触发雷暴效果（无伤害）
            triggerThunderstorm(drowned, target);
            
            if (currentTridents < minTridents) {
                int tridentsToThrow = minTridents - currentTridents;
                target.sendMessage("§c⚡ 溺尸王正在快速投掷 " + tridentsToThrow + " 把三叉戟！");
                
                // 快速连续投掷三叉戟
                for (int i = 0; i < tridentsToThrow; i++) {
                    final int index = i;
                    new BukkitRunnable() {
                        @Override
                        public void run() {
                            throwTrident(drowned, target);
                            if (index == tridentsToThrow - 1) {
                                // 最后一把三叉戟投掷后，延迟引爆
                                new BukkitRunnable() {
                                    @Override
                                    public void run() {
                                        explodeAllTridents(drowned, target);
                                    }
                                }.runTaskLater(AssassinPlugin.getInstance(), 10L); // 0.5秒后引爆
                            }
                        }
                    }.runTaskLater(AssassinPlugin.getInstance(), i * 5L); // 每0.25秒投掷一把
                }
            } else {
                // 已经有足够的三叉戟，直接引爆
                target.sendMessage("§6三叉戟狂欢节 ！");
                target.sendMessage("§c所有三叉戟即将爆炸！");
                
                // 播放警告音效
                target.getWorld().playSound(target.getLocation(), Sound.ENTITY_WITHER_SPAWN, 1.0f, 0.8f);
                ...
                }.runTaskLater(AssassinPlugin.getInstance(), 20L); // 1秒后引爆
            }
            
        } catch (Exception e) {
            AssassinPlugin.getInstance().getLogger().warning("触发三叉戟节日时出错: " + e.getMessage());
        }
    }
    
    // 触发雷暴效果（无伤害）
    private void triggerThunderstorm(Drowned drowned, Player target) {
        if (drowned == null || target == null) return;
        ...
        try {
            ...
            // 发送雷暴消息
            target.sendMessage("§9雷暴降临！");
            
            // 播放雷暴音效
            world.playSound(targetLoc, Sound.ENTITY_LIGHTNING_BOLT_THUNDER, 2.0f, 0.8f);
            ...
            for (int i = 0; i < 5; i++) {
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        double distance = 5 + Math.random() * 10; // 5-15格距离
                        double x = targetLoc.getX() + Math.cos(angle) * distance;
                        double z = targetLoc.getZ() + Math.sin(angle) * distance;
                        
                        // 找到地面高度
                        Location strikeLoc = new Location(world, x, world.getHighestBlockYAt((int)x, (int)z), z);
                        
                        // 生成无伤害的闪电效果
                        world.strikeLightningEffect(strikeLoc);
                        
                        // 播放音效
                        world.playSound(strikeLoc, Sound.ENTITY_LIGHTNING_BOLT_IMPACT, 1.0f, 1.0f);
                    }
                }.runTaskLater(AssassinPlugin.getInstance(), i * 10L); // 每0.5秒一道闪电
            }
            
        } catch (Exception e) {
            AssassinPlugin.getInstance().getLogger().warning("触发雷暴时出错: " + e.getMessage());
        }
    }
    
    // 引爆所有三叉戟
    private void explodeAllTridents(Drowned drowned, Player target) {
        List<UUID> tridents = drownedTridents.get(drowned.getUniqueId());
        if (tridents == null) return;
        ...
        // 复制列表以避免并发修改
        List<UUID> tridentsCopy = new ArrayList<>(tridents);
        
        for (UUID tridentId : tridentsCopy) {
            Entity trident = Bukkit.getEntity(tridentId);
            if (trident != null && trident.isValid()) {
                ...
                // 创建闪电苦力怕级别的大型爆炸
                createLightningCreeperExplosion(tridentLoc, drowned, target);
                explosionCount++;
        ...
        // 播放完成音效
        if (explosionCount > 0) {
            target.getWorld().playSound(target.getLocation(), Sound.ENTITY_GENERIC_EXPLODE, 1.5f, 0.8f);
            target.sendMessage("§c💥 " + explosionCount + " 把三叉戟同时爆炸！");
        }
    }
    
    // 创建闪电苦力怕级别的大型爆炸（摧毁方块）
    private void createLightningCreeperExplosion(Location location, Drowned drowned, Player target) {
        try {
            World world = location.getWorld();
            
            // 创建大型爆炸（闪电苦力怕级别）
            // 闪电苦力怕的爆炸威力是6，比普通苦力怕（3）强很多
            float explosionPower = 6.0f; // 闪电苦力怕的爆炸威力
            boolean setFire = false; // 是否生火
            boolean breakBlocks = true; // 摧毁方块
            
            // 创建爆炸
            world.createExplosion(location, explosionPower, setFire, breakBlocks, drowned);
            
            // 播放爆炸音效
            world.playSound(location, Sound.ENTITY_GENERIC_EXPLODE, 2.0f, 0.7f);
            
            // 对爆炸范围内的玩家造成25点伤害
            double explosionRadius = 8.0; // 闪电苦力怕的爆炸半径
            double fixedDamage = 25.0; // 固定25点伤害
            
            for (Entity entity : world.getNearbyEntities(location, explosionRadius, explosionRadius, explosionRadius)) {
                if (entity instanceof Player && !entity.equals(drowned)) {
                    Player nearbyPlayer = (Player) entity;
                    double distance = nearbyPlayer.getLocation().distance(location);
                    
                    if (distance <= explosionRadius) {
                        // 造成固定25点伤害
                        nearbyPlayer.damage(fixedDamage, drowned);
                        
                        if (nearbyPlayer.equals(target)) {
                            //nearbyPlayer.sendMessage("§c💥 你被三叉戟大爆炸击中！受到 " + fixedDamage + " 点伤害！");
                        }
                    }
                }
            }
            ...
            AssassinPlugin.getInstance().getLogger().warning("创建闪电苦力怕爆炸时出错: " + e.getMessage());
        }
    }
    ...
    private ItemStack createEnchantedTrident() {
        ItemStack trident = new ItemStack(Material.TRIDENT);
        ItemMeta tridentMeta = trident.getItemMeta();
        tridentMeta.addEnchant(Enchantment.CHANNELING, 1, true); // 引雷
        tridentMeta.addEnchant(Enchantment.IMPALING, 3, true);   // 穿刺III
        tridentMeta.setDisplayName("§b雷霆三叉戟");
        trident.setItemMeta(tridentMeta);
        return trident;
    }
    ...
    // 投掷三叉戟攻击 - 改进版本
    private void throwTrident(Drowned drowned, Player target) {
        if (drowned == null || target == null || drowned.isDead()) return;
        
        try {
            Location eyeLocation = drowned.getEyeLocation();
            Vector direction = target.getEyeLocation().toVector().subtract(eyeLocation.toVector()).normalize();
            ...
            // 创建三叉戟抛射物
            Trident trident = (Trident) drowned.getWorld().spawnEntity(
                eyeLocation.add(direction.multiply(2)), EntityType.TRIDENT
            );
            
            // 设置三叉戟属性
            trident.setShooter(drowned);
            trident.setVelocity(direction.multiply(1.5 + random.nextDouble() * 0.5)); // 随机速度
            trident.setDamage(8.0 + random.nextDouble() * 4.0); // 随机伤害8-12点
            
            // 设置三叉戟的附魔
            ItemStack tridentItem = createEnchantedTrident();
            trident.setItem(tridentItem);
            
            // 标记为溺尸王的三叉戟
            trident.getPersistentDataContainer().set(
                new NamespacedKey(AssassinPlugin.getInstance(), "drowned_trident"),
                PersistentDataType.STRING,
                drowned.getUniqueId().toString()
            );
            ...
            // 播放音效
            drowned.getWorld().playSound(drowned.getLocation(), Sound.ITEM_TRIDENT_THROW, 1.0f, 0.8f + random.nextFloat() * 0.4f);
            
        } catch (Exception e) {
            AssassinPlugin.getInstance().getLogger().warning("投掷三叉戟时出错: " + e.getMessage());
        }
    }
        ...
        while (iterator.hasNext()) {
            ...
            if (contract.isActive() && contract.getAssassinId() != null) {
                ...
                if (assassin == null || assassin.isDead()) {
                    ...
                    // 移除AI任务
                    if (assassin instanceof Drowned) {
                        BukkitRunnable aiTask = drownedAITasks.remove(assassin.getUniqueId());
                        if (aiTask != null) {
                            aiTask.cancel();
                        }
                    }
