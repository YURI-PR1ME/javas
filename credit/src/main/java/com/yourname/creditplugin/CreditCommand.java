...
public class CreditCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleCheck(CommandSender sender, String[] args) {
        ...
        String status = target.getGameMode() == GameMode.SPECTATOR ? ChatColor.RED + "死亡" : ChatColor.GREEN + "存活";
        
        sender.sendMessage(ChatColor.GREEN + "📊 " + target.getName() + " 的信用点: " + credits + " | 状态: " + status);
    }
    ...
    private void handleRemove(CommandSender sender, String[] args) {
        ...
        try {
            ...
            if (creditManager.removeCredits(target, amount)) {
                ...
                sender.sendMessage(ChatColor.RED + "❌ 信用点不足");
            }
        } catch (NumberFormatException e) {
            sender.sendMessage(ChatColor.RED + "❌ 请输入有效的数字");
        }
    ...
    private void handleRevive(CommandSender sender, String[] args) {
        ...
        target.sendMessage(ChatColor.GREEN + "✨ 你已被管理员复活！");
